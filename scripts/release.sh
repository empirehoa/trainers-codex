...
EOF
  ...
  dirty="$(git status --porcelain -- . ':!bundle.html')"
  if [[ -n "$dirty" ]]; then
    printf '%s\n' "$dirty" >&2
  ...
  if [[ $SKIP_INSTALL -eq 0 ]]; then
    ...
    run pnpm config set verify-deps-before-run false --location project || note "(older pnpm: setting not known, continuing)"
    run env PUPPETEER_SKIP_DOWNLOAD=1 pnpm install
    # worker/ has no committed lockfile (gitignored), so no --frozen-lockfile.
    run_in "$ROOT/worker" pnpm install --ignore-workspace
  fi
